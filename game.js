...
const AudioController = {
    ...
    playTheme(screen) {
        ...
        if (screen === 'arena') themeType = 'battle';
        if (['caves','mines','spire'].includes(screen)) themeType = 'dungeon';
    ...
    playNextNote(theme, index) {
        ...
        const scales = {
            peaceful: { notes: [261.63, 329.63, 392.00, 493.88, 392.00, 329.63, 293.66, 349.23], tempo: 600, type: 'sine', vol: 0.05 },
            battle: { notes: [110, 110, 110, 146.83, 110, 164.81, 155.56, 110, 110, 196.00], tempo: 150, type: 'sawtooth', vol: 0.04 },
            dungeon: { notes: [73.42, 0, 82.41, 0, 65.41, 0, 98.00], tempo: 800, type: 'triangle', vol: 0.08 }
        };
...
const ZONES = [
  { id: 'fields', name: 'Green Fields', minLvl: 1, enemies: [{ name: 'Garden Rat', level: 1, hp: 15, str: 2, def: 0, exp: 5, gold: 5 }, { name: 'Hungry Wolf', level: 3, hp: 35, str: 6, def: 2, exp: 15, gold: 15 }, { name: 'Road Bandit', level: 5, hp: 50, str: 10, def: 5, exp: 30, gold: 40 }] },
  { id: 'forest', name: 'Ancient Forest', minLvl: 11, enemies: [{ name: 'Giant Spider', level: 12, hp: 120, str: 20, def: 10, exp: 80, gold: 100 }, { name: 'Forest Troll', level: 15, hp: 200, str: 30, def: 15, exp: 120, gold: 180 }, { name: 'Corrupted Ent', level: 18, hp: 250, str: 40, def: 25, exp: 160, gold: 250 }] },
  { id: 'caves', name: 'Goblin Caves', minLvl: 21, enemies: [{ name: 'Goblin Scout', level: 22, hp: 300, str: 45, def: 20, exp: 250, gold: 350 }, { name: 'Cave Bat', level: 25, hp: 280, str: 55, def: 15, exp: 300, gold: 400 }, { name: 'Goblin King', level: 28, hp: 450, str: 65, def: 35, exp: 450, gold: 600 }] },
  { id: 'hills', name: 'Barrow Hills', minLvl: 31, enemies: [{ name: 'Restless Spirit', level: 32, hp: 500, str: 70, def: 40, exp: 600, gold: 800 }, { name: 'Barrow Wight', level: 35, hp: 650, str: 85, def: 50, exp: 750, gold: 1000 }, { name: 'Ancient King', level: 38, hp: 800, str: 100, def: 60, exp: 900, gold: 1200 }] },
  { id: 'mines', name: 'Deep Mines', minLvl: 41, enemies: [{ name: 'Orc Grunt', level: 42, hp: 1000, str: 120, def: 80, exp: 1200, gold: 1500 }, { name: 'Cave Troll', level: 45, hp: 1400, str: 150, def: 100, exp: 1500, gold: 2000 }, { name: 'Balrog Shadow', level: 48, hp: 1800, str: 180, def: 120, exp: 2000, gold: 3000 }] },
  { id: 'peaks', name: 'Snowy Peaks', minLvl: 51, enemies: [{ name: 'Frost Wolf', level: 52, hp: 2000, str: 200, def: 140, exp: 2500, gold: 3500 }, { name: 'Ice Giant', level: 55, hp: 2500, str: 240, def: 180, exp: 3000, gold: 4500 }, { name: 'Wyvern', level: 58, hp: 2200, str: 280, def: 150, exp: 3500, gold: 5000 }] },
  { id: 'swamp', name: 'Dead Marshes', minLvl: 61, enemies: [{ name: 'Marsh Ghoul', level: 62, hp: 3000, str: 320, def: 200, exp: 4500, gold: 6000 }, { name: 'Will-o-Wisp', level: 65, hp: 2800, str: 380, def: 100, exp: 5000, gold: 7000 }, { name: 'Hydra', level: 68, hp: 4000, str: 350, def: 250, exp: 6000, gold: 8000 }] },
  { id: 'fortress', name: 'Iron Fortress', minLvl: 71, enemies: [{ name: 'Uruk Berserker', level: 72, hp: 4500, str: 450, def: 300, exp: 7500, gold: 9000 }, { name: 'Dark Sorcerer', level: 75, hp: 3800, str: 550, def: 200, exp: 8500, gold: 10000 }, { name: 'War Elephant', level: 78, hp: 6000, str: 500, def: 400, exp: 10000, gold: 12000 }] },
  { id: 'volcano', name: 'Ash Plains', minLvl: 81, enemies: [{ name: 'Magma Sprite', level: 82, hp: 5500, str: 600, def: 350, exp: 12000, gold: 15000 }, { name: 'Fire Drake', level: 85, hp: 7000, str: 700, def: 450, exp: 15000, gold: 20000 }, { name: 'Obsidian Golem', level: 88, hp: 8000, str: 650, def: 600, exp: 18000, gold: 25000 }] },
  { id: 'spire', name: 'The Dark Spire', minLvl: 91, enemies: [{ name: 'Nazgul', level: 92, hp: 9000, str: 800, def: 500, exp: 25000, gold: 35000 }, { name: 'Dark Lord', level: 95, hp: 12000, str: 900, def: 600, exp: 35000, gold: 50000 }, { name: 'The Eye', level: 100, hp: 20000, str: 1200, def: 800, exp: 50000, gold: 100000 }] }
];
...
window.actions = {
        ...
        try {
            if (state.authForm.isRegistering) {
                ...
                const initialProfile = {
                    ...
                    lastSeen: serverTimestamp(), allowPvp: true // Default PvP on
                };
    ...
    enterArena: async () => {
        ...
        updateState({ screen: 'arena', arenaList: [] }); // Clear old list
        
        try {
            // Get recent users. Note: Complex sorting might require index, so we get 20 and sort in JS
            ...
            snap.forEach(doc => {
                const d = doc.data();
                if(doc.id !== state.user.uid) { // Don't show self
                    players.push({ uid: doc.id, ...d });
                }
            });
            
            // Filter: Active recently (simulated by checking if lastSeen exists)
            // Sort by level desc
    ...
    challengePlayer: async (opponentUid) => {
        ...
        if(!opponent) return;
        if(!opponent.allowPvp) { alert("This player has disabled challenges."); return; }
        if(state.player.energy < 5) { alert("Too tired for PvP (5 Energy)."); return; }
        ...
        updateState({
            ...
            combat: {
                ...
                enemy: { 
                    ...
                    hp: opponent.maxHp, // They start full HP in simulation
                    maxHp: opponent.maxHp, 
                    str: opponent.str, 
                    def: opponent.def, 
                    exp: opponent.level * 10, // Reward calculation
                    gold: opponent.level * 5 
                },
                log: [`You challenged ${opponent.name} to a duel!`],
                round: 1
            }
        });
    },
    ...
    sendCourierMessage: async () => {
        if(!state.mailTo.trim() || !state.mailBody.trim()) { alert("Fill all fields."); return; }
        AudioController.playSFX('click');
    ...
    combatRound: async (action) => {
        ...
        if (enemy.hp <= 0) {
            ...
            if(newExp >= player.expToNext) {
                newLevel++; newExp -= player.expToNext; newMaxHp += 15;
                msg += " LEVEL UP!";
                ...
                if (snap.exists()) {
                    ...
                    if(!window.regenInterval) {
                        window.regenInterval = setInterval(() => {
                            ...
                            if(p) setDoc(getPlayerRef(u.uid), { 
                                energy: Math.min(p.energy + 1, p.maxEnergy),
                                lastSeen: window.FB.serverTimestamp() // Update online status
                            }, { merge: true });
                        }, 10000);
                    }
...
function render() {
    ...
    if (state.screen === 'auth') {
        ...
        <div style="margin-top:15px; font-size: 0.8rem; cursor:pointer;" onclick="window.actions.setAuthMode(!state.authForm.isRegistering)">${state.authForm.isRegistering ? 'Already have a hero? Login' : 'Need a hero? Register'}</div>
        </div></div>`;
        return;
    }
    ...
    if (state.screen === 'home') {
        ...
        ZONES.map(zone => {
            ...
            return `<div onclick="${locked ? '' : `window.actions.enterZone('${zone.id}')`}" class="card" style="cursor: ${locked ? 'default' : 'pointer'}; opacity: ${locked ? '0.6' : '1'}; display:flex; justify-content:space-between; align-items:center;">
